...
select
  ...
  count(case when status_odp LIKE 'odrzuco%' then o.id end)/count(a.id)::numeric proc_odrzuconych_odp_operator, -- procent odrzuconych z odpowiedzi operatora
  ...
  (count(o.id)-count(ap.id))/count(o.id)::numeric proc_drop_off_odp_operator  --dropp off czyli procent króry nie przeszedł do analizy sądowej
